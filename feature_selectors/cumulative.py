...
class CumulativeFeatureSelector(BaseEstimator, TransformerMixin):
    ...
    Each stage can be toggled for ablation.
    """
    ...
    def __init__(
        ...
        self.stage_features_: dict[str, list[str]] = {}
        self.importance_series_: pd.Series | None = None
    ...
    def _log_stage(self, label: str, before: int, after: int, names: list[str] | None = None):
        if not self.verbose:
            return
        dropped = before - after
        print(f"  - {label}: {before} -> {after} features (dropped {dropped})")
        if names:
            preview = ", ".join(names[:20])
            suffix = " ..." if len(names) > 20 else ""
            print(f"    kept: {preview}{suffix}")
    ...
    def fit(self, X, y: pd.Series, **fit_params):
        ...
        if self.verbose:
            print("\n🔥 Starting Cumulative Feature Selection "
        ...
        if self.use_variance:
            ...
            self.stage_features_["variance"] = X_stage1.columns.tolist()
            self._log_stage(f"Stage 1 (Variance, thresh={self.variance_threshold})",
                            initial_count, X_stage1.shape[1], self.stage_features_["variance"])
        else:
            X_stage1 = X_df
            if self.verbose:
        ...
        if self.use_importance:
            if self.verbose:
                print(
                    f"  - Stage 2 (Cumulative Importance): Pre-selecting "
                    f"from {X_stage1.shape[1]} features..."
                )
            ...
            self.importance_series_ = importances

            if importances.empty:
                print(
                    "  [Selector] Warning: No feature importance > 0. "
                    "Keeping the best single feature."
                )
            ...
            self.stage_features_["importance"] = selected_features_stage2
            self._log_stage(f"Stage 2 (Cumulative Importance, q={self.importance_quantile})",
                            X_stage1.shape[1], X_stage2.shape[1], selected_features_stage2)
        else:
            X_stage2 = X_stage1
            if self.verbose:
        ...
        if self.use_corr:
            if self.verbose:
                print(
                    f"  - Stage 3 (Correlation Filter, thresh={self.corr_threshold}): "
            ...
            self.stage_features_["correlation"] = X_final.columns.tolist()
            self._log_stage(f"Stage 3 (Correlation Filter, thresh={self.corr_threshold})",
                            X_stage2.shape[1], X_final.shape[1], self.stage_features_["correlation"])
            if self.verbose:
                print(f"  ✅ [Selector] Completed. Final count: {initial_count} -> {X_final.shape[1]} features "
            ...
            if self.verbose:
                ...
                print(
                    f"  ✅ [Selector] Completed. Final count: "
                    f"{initial_count} -> {X_final.shape[1]} features (no correlation pruning)"
                )
    ...
    def transform(self, X) -> pd.DataFrame:
        ...
        if missing_cols:
            print(
                f"Warning: {len(missing_cols)} selected columns missing in new data. "
